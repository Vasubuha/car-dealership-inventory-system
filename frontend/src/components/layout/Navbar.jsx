...
export default function Navbar({ onMenu }) {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-20 items-center gap-3 border-b border-slate-100 bg-white/90 px-5 backdrop-blur lg:px-8">
        ...
        <div className="relative">
          ...
          >
            ...
            <span className="hidden text-left sm:block">
              <b className="block max-w-32 truncate text-sm text-slate-800">
                {user?.username || 'My account'}
              </b>
              <small className="capitalize text-slate-500">{user?.role || 'customer'}</small>
            </span>
