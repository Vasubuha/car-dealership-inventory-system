...
function userFromToken(token: string): AuthUser | null { try { const payload = JSON.parse(atob(token.split('.')[1].replace(/-/g, '+').replace(/_/g, '/'))); return { id: payload.user_id ?? payload.sub, username: payload.username, email: payload.email, role: payload.role ?? 'customer' } } catch { return null } }
